...
const getAll = async () => {
  const [rows] = await db.execute('SELECT a.id,a.phieu_nhap_id,a.thiet_bi_id,a.loai_thiet_bi_id,a.don_vi_tinh_id,b.ma_phieu_nhap,c.ten_thiet_bi,d.ten_loai,e.ten_don_vi_tinh,a.so_luong,a.don_gia,a.thanh_tien FROM camera_app.chi_tiet_phieu_nhap a inner join camera_app.phieu_nhap b on a.phieu_nhap_id=b.id inner join camera_app.thiet_bi c on a.thiet_bi_id=c.id inner join camera_app.loai_thiet_bi d on a.loai_thiet_bi_id=d.id inner join camera_app.don_vi_tinh e on a.don_vi_tinh_id=e.id ORDER BY a.created_at ASC');
  return rows;
};
